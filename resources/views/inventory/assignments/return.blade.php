...
@section('header')
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Process Return - Assignment #{{ $assignment->id }}
        </h2>
...
@section('content')  
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex-shrink-0 h-12 w-12">
                                <div class="h-12 w-12 rounded-full {{ $assignment->staff->isMilitary() ? 'bg-gradient-to-r from-green-600 to-green-800' : 'bg-gradient-to-r from-blue-500 to-purple-600' }} flex items-center justify-center">
                            ...
                            <div>
                                <h4 class="font-semibold text-gray-900">{{ $assignment->staff->name }}</h4>
                                <p class="text-sm text-gray-600">{{ $assignment->staff->service_number }}</p>
                                <p class="text-xs text-gray-500">{{ $assignment->staff->department }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h4 class="font-semibold text-gray-900">{{ $assignment->item->name }}</h4>
                                <p class="text-sm text-gray-600">{{ $assignment->item->item_code }}</p>
                                <p class="text-xs text-gray-500">Quantity: {{ $assignment->quantity }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                            ...
                            <span class="text-gray-600">{{ $assignment->assigned_date->format('M j, Y') }}</span>
                        </div>
                        @if($assignment->expected_return_date)
                            <div>
                                <span class="font-medium text-gray-900">Expected Return:</span>
                                <span class="text-gray-600 {{ $assignment->isOverdue() ? 'text-red-600 font-semibold' : '' }}">
                        ...
                        <div>
                            ...
                            <span class="text-gray-600">{{ $assignment->assignedBy->name }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('inventory.assignments.process-return', $assignment) }}" class="space-y-6">
                        @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="quantity_returned" id="quantity_returned" 
                                           value="{{ old('quantity_returned', $assignment->quantity) }}" 
                                           min="1" max="{{ $assignment->quantity }}" required
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <p class="mt-1 text-xs text-gray-500">
                                        Maximum: {{ $assignment->quantity }} (originally assigned)
                                    </p>
                                ...
                                <div>
                                    ...
                                    <select name="condition_on_return" id="condition_on_return" required
                                        ...
                                        <option value="good" {{ old('condition_on_return') == 'good' ? 'selected' : '' }}>Good</option>
                                        <option value="fair" {{ old('condition_on_return') == 'fair' ? 'selected' : '' }}>Fair</option>
                                        <option value="poor" {{ old('condition_on_return') == 'poor' ? 'selected' : '' }}>Poor</option>
                                        <option value="damaged" {{ old('condition_on_return') == 'damaged' ? 'selected' : '' }}>Damaged</option>
                                    </select>
                                    <p class="mt-1 text-xs text-gray-500">
                                        Original condition: <strong>{{ ucfirst($assignment->item->condition) }}</strong>
                                    </p>
                        ...
                        <div class="border-b border-gray-200 pb-6">
                            ...
                            <div>
                                ...
                                <textarea name="return_notes" id="return_notes" rows="4"
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                          placeholder="Any issues, damages, or observations about the returned item...">{{ old('return_notes') }}</textarea>
                                <p class="mt-1 text-xs text-gray-500">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            ...
                            <div id="return-summary" class="text-sm text-gray-600 space-y-1">
                                ...
                                <p><strong>Item:</strong> {{ $assignment->item->name }} ({{ $assignment->item->item_code }})</p>
                                <p><strong>Quantity Returning:</strong> <span id="summary-quantity">{{ $assignment->quantity }}</span> of {{ $assignment->quantity }}</p>
                                <p><strong>Condition:</strong> <span id="summary-condition">Not selected</span></p>
                                <p><strong>Return Date:</strong> {{ now()->format('M j, Y') }}</p>
    ...
    <script>
        ...
        function updateSummary() {
            ...
            document.getElementById('summary-quantity').textContent = quantity || '{{ $assignment->quantity }}';
