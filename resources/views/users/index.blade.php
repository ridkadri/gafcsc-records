                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <div class="flex items-center space-x-4 mt-1 text-sm text-gray-500">
                                ...
                                <span>•</span>
                                <span class="text-red-600">Super Admins: {{ $users->where('role', 'super_admin')->count() }}</span>
                                <span>•</span>
                                <span class="text-green-600">Admins: {{ $users->where('role', 'admin')->count() }}</span>
                                <span>•</span>
                                <span class="text-gray-600">Viewers: {{ $users->where('role', 'viewer')->count() }}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($users as $user)
                                    <tr class="hover:bg-gray-50 {{ $user->isSuperAdmin() ? 'bg-red-50' : '' }}">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10">
                                                    <div class="h-10 w-10 rounded-full {{ $user->isSuperAdmin() ? 'bg-gradient-to-r from-red-500 to-red-600' : 'bg-gradient-to-r from-blue-500 to-purple-600' }} flex items-center justify-center">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                <!-- View Button -->
                                                <a href="{{ route('users.show', $user) }}" class="text-blue-600 hover:text-blue-900" title="View Details">
                                                ...
                                                @if(auth()->user()->canEdit($user))
                                                    <a href="{{ route('users.edit', $user) }}" class="text-indigo-600 hover:text-indigo-900" title="Change Role">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                @if(auth()->user()->canResetPassword($user))
                                                    <a href="{{ route('users.reset-form', $user) }}" class="text-yellow-600 hover:text-yellow-900" title="Reset to Default Password">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($users->isEmpty())
                        <div class="text-center py-12">
                            ...
                            <div class="mt-6">
                                <a href="{{ route('users.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                                    Create User
                                </a>
                            </div>
                        </div>
                    ...
                    <!-- Protection Notice -->
                    @if($users->where('role', 'super_admin')->count() > 0)
